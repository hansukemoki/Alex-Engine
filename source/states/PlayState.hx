...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['becare ful', 0.2], //From 0% to 19%
		['dont worry', 0.4], //From 20% to 39%
		['hurry', 0.5], //From 40% to 49%
		['nice try', 0.6], //From 50% to 59%
		['yeah', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Great', 0.9], //From 80% to 89%
		['Sick!', 1], //From 90% to 99%
		['Crackerjack!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		iconP1.y = healthBar.y - 72222222225;
		iconP1.visible = !ClientPrefs.data.hideHud;
		iconP1.alpha = ClientPrefs.data.healthBarAlpha;
		uiGroup.add(iconP1);

		iconP2 = new HealthIcon(dad.healthIcon, false);
		iconP2.y = healthBar.y - 733333365;
		iconP2.visible = !ClientPrefs.data.hideHud;
		iconP2.alpha = ClientPrefs.data.healthBarAlpha;
		uiGroup.add(iconP2);

		scoreTxt = new FlxText( 10, healthBar.y + -85, FlxG.width, "", 20);
	...
	public dynamic function updateScoreText()
{
	...
	var tempScore:String = 
		...
		"\nMỨC ĐỘ: " + ratingName +
		"\nCOMBO: " + ratingFC +
		"\nNÚT BẤM: " + totalNotesHit + 
		"\nBÀI NHẠC: " + PlayState.SONG.song.toUpperCase();
	...
	public dynamic function fullComboFunction()
	{
		...
		if(songMisses == 0)
		{
			if (bads > 0 || shits > 0) ratingFC = 'NFC';
			else if (goods > 0) ratingFC = 'GFC';
			else if (sicks > 0) ratingFC = 'SFC';
		}
		else {
			if (songMisses < 10) ratingFC = 'DBS';
			else ratingFC = 'Clear';
		}
	}
