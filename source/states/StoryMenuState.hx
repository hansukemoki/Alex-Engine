...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(10, 10, 0, Language.getPhrase('week_score', 'ĐIỂM CỦA TUẦN: {1}', [lerpScore]), 36);
		...
		var bgYellow:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 500, 0xFF000000);
		bgSprite = new FlxSprite(0, 175);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			if(!isLocked || !weekFile.hiddenUntilUnlocked)
			{
				...
				itemTargetY += Math.max(weekThing.height, 110) + 900;
		...
		leftArrow = new FlxSprite(450, grpWeekText.members[0].y + -550);
	...
	override function update(elapsed:Float)
{
    ...
    // ==== Di chuyển txtTracklist từ trái sang phải ====
    var speed:Float = 100; // px mỗi giây
    txtTracklist.x += speed * elapsed;

    // Nếu chạy hết bên phải màn hình thì quay lại trái
    if (txtTracklist.x > FlxG.width)
    {
    ...
    if(WeekData.weeksList.length < 1)
    {
		...
		if(intendedScore != lerpScore)
		{
			...
			scoreText.text = Language.getPhrase('week_score', 'ĐIỂM CỦA TUẦN: {1}', [lerpScore]);
		}
...
{
    // Chỉnh Y (cao/thấp)
    item.y = FlxMath.lerp(item.targetY + 580, item.y, Math.exp(-elapsed * 10.2));

    // Chỉnh X (trái/phải) - dùng vị trí ban đầu làm "target"
    var baseX:Float = 100; // hoặc giá trị x gốc của bạn
    item.x = FlxMath.lerp(baseX + 800, item.x, Math.exp(-elapsed * 10.2));
}
	...
	function updateText()
	{
		...
		{
			var lWeek:WeekData = loadedWeeks[curWeek];
            txtTracklist.text = lWeek.songs.join(" ");
		}
